...
NAME=${NAME:-"PRO-API-01A"} # Instance name in AWS console
REGION=${REGION:-"ap-southeast-1"} # Region
VPC_ID=${SVPC_ID:-"vpc-07a3c563"}
SUBNET_ID=${SUBNET_ID:-"subnet-b49dbfd0"} # VPC Subnet
INSTANCE_TYPE=${INSTANCE_TYPE:-"t2.medium"}  # Instance type
VOLUME_SIZE=${VOL:-200}  # Root size
VOLUME_TYPE=${TYPE:-"GP2"}  # Root size
IAM_PROFILE=${IAM_PROFILE:-"PRO-EC2"} # IAM Role
SECURITY_GROUP_ID=${SECURITY_GROUP_ID:-"sg-7c42391b"} # Security group
ENVIRONMENT=${ENVIRONMENT:-"Production"}
VPC=${VPC:-"CML"}
PURPOSE=${PURPOSE:-"Logstash"}
